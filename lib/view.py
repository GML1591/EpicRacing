    ...
    def draw(self, state):
        ...
        if state == STATE_PLAY and self.isPlaying:
            ...
            while True:
                x = defaultx
                while True:
                    drawx = -camera_rel_pos[0] + x # sistema de coordenadas relativo al jugador
                    ...
                    if -bg_size[0] < drawx < self.windowWidth and -bg_size[1] < drawy < self.windowHeight:
                        self.screen.blit(fondo, (drawx, drawy)) # dibuja los objetos basados en esa posicion (drawx,drawy)
                    x += bg_size[0]
                    if x >= max_x:
                        break
                y += bg_size[1]
                if y >= max_y:
                    break
            # Se dibujan las marcas del terreno, esto fue lo que mas me costo
            ...
            if self.show_track:
                for track in self.map.get_track(): # la lista que contiene cada trozo de la pista, obviamente deben formar un ciclo
            ...
            # Si no se ha terminado la pista (el jugador desaparece cuando termina de recorrer las vueltas)
