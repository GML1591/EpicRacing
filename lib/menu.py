        ...
        for option in self.actual.opciones:
            ...
            if option[0] == SELECTOR:
                ...
                if dy == self.actual.index:
                    text = self.actual.font.render(decode_uf8(option[1].get()), 1,
                    ...
                    text_bg = self.actual.font.render(decode_uf8(option[1].get()), 1,
                                                      SHADOW)
            else:
                # Si el indice seleccionado es el item se cambia el color
                if dy == self.actual.index:
                    text = self.actual.font.render(decode_uf8(option[0]), 1,
                                                   self.actual.selectedcolor)
                    text_bg = self.actual.font.render(decode_uf8(option[0]), 1, SHADOW)
                else:
                    text = self.actual.font.render(decode_uf8(option[0]), 1,
                                                   self.actual.fontColor)
                    text_bg = self.actual.font.render(decode_uf8(option[0]), 1, SHADOW)
