...
class Langloader(object):
    ...
    def __init__(self, language, **kwargs):
        ...
        if language + langconfig.getValue(0) in langavaiable.getParameters():
            try:
                # file = open(_LANG_DIRLANGS + language + langconfig.getValue(0))  # @ReservedAssignment
                file = io.open(_LANG_DIRLANGS + language + langconfig.getValue(0), 'r', encoding='utf8')
            except:
            ...
            for line in file:
                ...
                # if line[0] == "":
                #     line[0] = "10"
                line = line.strip().split(_SPLITTER)
                self.lang[int(line[0].replace("\ufeff", ""))] = decode_uf8(line[1].replace(
                    _SPACE, " "))
            file.close()
